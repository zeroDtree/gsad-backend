...
class UserImportServiceTest {
    ...
    void importCsv_createsUserWithProvidedPassword() {
        ...
        when(applicationPasswordGenerator.resolvePassword("TempPass2024!")).thenReturn("TempPass2024!");
        ...
        assertThat(response.passwords()).isEmpty();
    ...
    @Test
    void importCsv_generatesPasswordWhenMissing() {
        String csv = """
                email,linux_username,display_name,student_id,cohort,initial_password,roles
                bob@example.com,bob,,,,,
                """;
        ...
        when(userRepository.existsByLinuxUsername("bob")).thenReturn(false);
        when(linuxUsernameResolver.validateAndReturn("bob")).thenReturn("bob");
        when(applicationPasswordGenerator.resolvePassword("")).thenReturn("generated-secret");
        when(passwordEncoder.encode("generated-secret")).thenReturn("bcrypt-hash");
        when(userRepository.save(any(User.class))).thenAnswer(invocation -> invocation.getArgument(0));

        UserImportResponse response = userImportService.importCsv(file);

        assertThat(response.created()).isEqualTo(1);
        assertThat(response.passwords()).hasSize(1);
        assertThat(response.passwords().getFirst().email()).isEqualTo("bob@example.com");
        assertThat(response.passwords().getFirst().initialPassword()).isEqualTo("generated-secret");
    }

    @Test
    void importCsv_skipsExistingEmail() {
        String csv = """
                email,linux_username
